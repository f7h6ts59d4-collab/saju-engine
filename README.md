...
절기 경계 보정 명식 계산(`correctPillars`)은 `src/correction/` 아래 11개 모듈로 구성됩니다. 작업 시 어느 파일을 봐야 할지 기준으로 삼으세요.
...
- `correct-pillars` → `timezone`, `ten-gods`, `sinsal`, `twelve-stages`, `relations`, `strength`, `month-year-pillar`, `major-luck`, `true-solar-time` (+ `core/saju`, `core/solar-lunar-converter`, `data/sixty-pillars`)
