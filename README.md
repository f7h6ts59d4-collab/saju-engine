...
절기 경계 보정 명식 계산(`correctPillars`)은 `src/correction/` 아래 12개 모듈로 구성됩니다. 작업 시 어느 파일을 봐야 할지 기준으로 삼으세요.
...
| `relations.ts` | 합충형파해(合沖刑破害)·공망 판정 — 천간합·천간충(10쌍)·육합·삼합·방합(반합 포함)·충·형·파·해·원진 + 순중공망. 규칙표는 포스텔러 관측 검증값 하드코딩(`computeRelations`, `computeGongmang`) |
| `strength.ts` | 신강/신약(身强/身弱) 판정 — 억부 점수제(아군/적군 자리 가중치 합산), 통근 보너스(건록·제왕 가중), 합반·충 50% 감점(relations 출력 사용), 무근 게이트, 7단계 라벨 + 조후 별도 필드(`computeStrength`) |
| `god-states.ts` | 십성 기운(투출·통근) 판정 — 다섯 십성 그룹의 천간 투출·지지 뿌리(궁위·층·조습 플래그) 조회 + 4단 상태(투출유근/투출무근/잠복/무) 분류(`computeGodStates`) |
...
- `correct-pillars` → `timezone`, `ten-gods`, `sinsal`, `twelve-stages`, `relations`, `strength`, `god-states`, `month-year-pillar`, `major-luck`, `true-solar-time` (+ `core/saju`, `core/solar-lunar-converter`, `data/sixty-pillars`)
...
- `strength` → `ten-gods`, `twelve-stages` (`relations`는 `Relation` 타입만 참조라 런타임 의존 없음)
- `god-states` → `ten-gods`
- `true-solar-time` → astronomy-engine
- `solar-terms-table` → `solar-terms-precise.json`
- `ten-gods`, `timezone`, `sinsal`, `twelve-stages`, `relations` → 내부 의존 없음(리프 모듈)
...
- **합충형파해(relations)**: 천간합·천간충·육합·삼합·방합(반합)·충·형·파·해·원진 — 겹침 허용, `adjacent` 인접 표시, 시간 모름이면 시주 제외
- **공망(gongmang)**: 일주 순중공망 지지 2개(`branches`) + 해당 기둥(`positions`, 연·월·시지 판정)
- **신강/신약(strength)**: 억부 점수(`score`)·판정 근거(`factors`: 득령·득지·통근·감점)·7단계 라벨·희기 방향(`favorable`) + 조후(`johu`, 점수 미합산 별도 필드) — 시간 모름이면 시주 제외, 성별과 무관
- **십성 기운(godStates)**: 다섯 십성 그룹(비겁·식상·재성·관성·인성)의 투출(`exposed`)·뿌리(`rooted`: 궁위·층·조습 플래그)·4단 상태(`state`) — 시간 모름이면 시주 제외
